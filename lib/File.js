...
// (File.readSync filePath:String callback:(s:String->undefined)) : undefined
function read(PROCESS, RUNTIME) {
    ...
    let fullPath = path.join(RUNTIME.workingDir, filePath);

    console.log(fullPath);

    PROCESS.Step(); // 立刻退出，执行下一指令

    fs.readFile(fullPath, {encoding:"utf-8"}, (error, data)=> {
        if(error) {
...
function readSync(PROCESS, RUNTIME) {
    ...
    let filePath = TrimQuotes(PROCESS.heap.Get(filePathHandle).content);
    let fullPath = path.join(RUNTIME.workingDir, filePath);
    let data = fs.readFileSync(fullPath, {encoding:"utf-8"}).toString();
    // 构造字符串对象
    let strHandle = PROCESS.heap.AllocateHandle("STRING", false);
    let strObject = {
        type: "STRING",
...
function writeString(PROCESS, RUNTIME) {
    ...
    fs.writeFile(fullPath, strdata, {encoding:"utf-8", flag: flag}, (error)=> {
        if(error) {
            PROCESS.OPSTACK.push("#f"); // TODO native函数的错误处理仍需细化
        }
        else {
...
function writeStringSync(PROCESS, RUNTIME) {
    ...
    fs.writeFileSync(fullPath, strdata, {encoding:"utf-8", flag: flag});
    PROCESS.Step();
}
