...
function read(PROCESS, RUNTIME) {
    ...
    let filePath = _ANIMAC_NATIVE_UTILS.TrimQuotes(PROCESS.heap.Get(filePathHandle).content);
...
function writeString(PROCESS, RUNTIME) {
    ...
    let flag = _ANIMAC_NATIVE_UTILS.TrimQuotes(PROCESS.heap.Get(flagHandle).content) || "w";

    let strdataHandle = PROCESS.PopOperand();
    let strdata = _ANIMAC_NATIVE_UTILS.TrimQuotes(PROCESS.heap.Get(strdataHandle).content);
