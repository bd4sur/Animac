...
(define run
  (lambda () {
    (display "基于FFT的大整数乘法\n")
    (display "预期结果（直接数字相乘） = ")
    (define expected (String.atom_to_string (* 114514 1919810)))
    (display expected)
    (newline)
    (display "实际结果（大数乘法算法） = ")
    (define output (big_int_multiply "114514" "1919810"))
    (display output)
    (newline)
    (if (equal? expected output)
      (display "✅ PASS big_int\n")
      (display "❌ FAIL big_int\n"))
    (newline)
  })
)
