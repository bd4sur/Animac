...
- GC 触发策略为“堆水位为主、周期兜底为辅”的三级触发（2026-07 起，取代原先的每轮事件循环定时 GC）：
  ...
  - L1（runtime 层）：`am_runtime_tick` 内每 `AM_GC_WATERMARK_CHECK_STRIDE` 条指令及 tick 末尾检查水位（`runtime_gc_watermark_check`），级别 1 做标记-清除、级别 2 强制压缩；发现 oom_flag 也强制一轮 GC 以挽救其余进程。水位兼顺两个维度：用量比（used/capacity）与碎片（最大空闲块低于容量 1/32 且用量≥30%，防止 first-fit 碎片化失败）。
  - L2（事件循环层）：每 `AM_GC_PERIODIC_INTERVAL` 轮事件循环执行一轮兜底 GC（0 表示禁用，即纯水位）。
  - allocator 新增查询：`am_allocator_heap_usage`（used/capacity/largest_free_block）、`am_allocator_heap_take_oom_flag`。
