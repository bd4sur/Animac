...
async function _generate(prompt, args, on_ready, on_running, on_finished) {
    if(is_generating) {
        return;
    }
    is_generating = true;
...
// (LLM.init modelFileBase64:string prompt:string) : void
function init(PROCESS, RUNTIME) {
    // 从栈中获取参数，注意顺序是反的
    let promptHandle = PROCESS.PopOperand();
    let promptStr = TrimQuotes(PROCESS.heap.Get(promptHandle).content);
    ...
    load_model_from_base64(modelFileBase64);
    args = {
        top_p:              0.5,
        top_k:              0,
        temperature:        1.0,
...
// (LLM.generate) : string
function generate(PROCESS, RUNTIME) {
    ...
    PROCESS.OPSTACK.push(outputStrHandle);
    PROCESS.Step(); // 退出，执行下一指令
}
